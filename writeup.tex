...
\begin{proof}[of Lemma~\ref{lem:nongaussian_nonindep}]
    \comment{calculations with characteristic functions?}
\end{proof}

This result should perhaps not be surprising:
there is a deep connection between independence and the Gaussian distribution.
For instance, a proof of the basic properties of the (Gaussian) infinitesimal model
...
\end{align*}
Suppose we then can prove (as is indeed true in the Gaussian case) that
(a) $(U_M^{[1]}, V_M^{[1]}, U_M^{[2]}, V_M^{[2]})$ converge in distribution;
(b) the limiting random variables are independent; and
defining $R$, $Z^{[1]}$ and $Z^{[2]}$ as is natural in the limit,
(c) $R$ is independent of $Z^{[1]}$ and $Z^{[2]}$.
...
It is not hard to see that (c) implies that $U^{[1]}$ and $V^{[1]}$
...
\begin{lemma}\label{lem:parental_contribs}
    Under the conditions of Lemma~\ref{lem:nongaussian_nonindep},
    $U^{[1]}$ and $V^{[1]}$ are not independent.
    \comment{OR SOMETHING?}
\end{lemma}

\begin{proof}[of Lemma~\ref{lem:parental_contribs}]
    \comment{more calcs with characteristic functions?}
\end{proof}
