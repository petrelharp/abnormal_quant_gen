...
\comment{
    Proposal: describe model, pointing out it's the infinitesimal model with gaussian noise;
    say something about the Cauchy case (maintains diversity with blending inheritance?);
    and jump to equation (1);
    then present section 5.4 which we use to parameterize simulation.
}

As observed in \cite{turelli2017commentary}, there are at least three interpretations of what is called ``the infinitesimal model''.  The maximalist of these, what \cite{turelli2017commentary} calls the Gaussian-population approximation, assumes a large number of small-effect loci,  parental trait-independent Gaussian Mendelian sampling terms  (as rigorously justified in \cite{barton2017infinitesimal,barton2022infinitesimal}), and, moreover, that  population trait values are in deterministic mutation-selection balance, with a Gaussian empirical distribution (\ie the \emph{fraction} of the population with trait value in $[z,z+dz)$ is approximately $\frac{1}{2\pi\sigma^{2}} e^{-\frac{(z-m)^{2}}{2\sigma^{2}}}$ for suitable choices of the empirical mean and variance $m$ and $\sigma^{2}$.  Leaving aside -- as the Gaussian-population approximation frequently does -- the question of whether or not some combination of effect sizes, dominance, and epistasis can justify non-Gaussian, trait-independent,  Mendelian sampling terms, we might reasonably ask when (or if) deterministic mutation selection-balance is possible in an non-Gaussian model.  

Concretely, consider a  Moran process where an individual whose trait value is $Z$ dies at rate $\mu(Z)$ and is replaced by a new individual of phenotype
\begin{align*}
  \frac{Z^{1} +Z^{2}}{2} + R ,
\end{align*}
where $Z^{1}$ and $Z^{2}$ are independent draws from the empirical population distribution and $R$ is an independent draw from the transmission noise distribution (independent of $Z^{1}$ and $Z^{2}$).  Mutation selection balance demands that \comment{I'm struggling a bit to find a clear, short, and simple argument to jump directly to this point} that $\frac{Z^{1} +Z^{2}}{2} + R$ be equal in distribution to the selection-biased choice of $Z$, \ie for all bounded and continuous functions $f$,
...
\begin{align*}
	\frac{\E\left[ f(Z) \mu(Z) \right]}{\E[\mu(Z)]} &= \E\left[ e^{i\theta\left(\frac{Z^{1} +Z^{2}}{2} + R \right)}\right]\\
	&= \E\left[ e^{i\frac{theta}{2} Z^{1}}e^{i\frac{theta}{2} Z^{2}}e^{\i\theta R}\right]\\
...
\[
	\phi(\theta) = \lim_{n \to \infty} F^{(n)}(\varphi)(\theta).
\]
...
\]
We will briefly explore what limiting values are possible -- and which limits are valid characteristic functions.  We refer to the first term on the left hand side as the \emph{reproduction term} and the remaining sum as the \emph{noise term}. For the sake of simplicity, we will limit ourselves to the caae when both have well-posed limits independent of the other.

\subsubsection{Reproductive Terms}

First consider the limit
\[
		\alpha(\theta) = \lim_{n \to \infty} 2^{n}\psi\left(\frac{\theta}{2^{n}}\right).
\]
Because $\psi(\theta)$ is the logarithm characteristic function, it must be continuous in some neighbourhood of, and vanish at 0, and thus $\alpha(\theta)$ as well.  Furthermore, for all integers $k$, 
\[
...
\]
as $k \to - \infty$ if the latter exists (\ie there is a unique left-hand limit at zero; this is the case if \eg we require $\alpha(\theta)$, and thus $\phi(\theta)$, to be convex functions).  In this case, setting $-\gamma + i m = \alpha'(0+)$, we have
\[
