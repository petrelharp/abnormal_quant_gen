...
on spatial genetic and dynamics \comment{I'm not quite sure what you mean by that}
...
\]
Because $\psi(\theta)$ is the logarithm of a characteristic function, it is continuous in some neighbourhood of, and vanishes at 0.
...
while is meaningless to speak of variances when the transmission noise is $\alpha$-stable, if an individual's trait value is informative of the allelic effect at any given locus,
...
To see what knowing the parental trait value $Z^{[j]}_{M}$ tells us about the contribution of any given locus, $\eta^{[j]}_{\ell}$, we follow the arguments in  \citet{Fisher1918} and \citet{barton2017infinitesimal} using Bayes' Theorem. Proceeding informally, with the probabilities indicating probability density functions, we have 
\begin{align*}
