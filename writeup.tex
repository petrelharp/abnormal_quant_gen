...
\citet{barton2017infinitesimal} give an overview of the complex history of the model, \revpoint{1}{12}
...
Oth the other hand, if $\alpha \ge 2$ then the distribution has finite variance, \revpoint{2}{1} 
...
On the other hand, the largest of $n$ Gaussians is of order $\sqrt{\log n}$. \revpoint{1}{11}
...
in fact, we will see that independence of offsprings' deviations and midparent values \revpoint{1}{2}
...
In fact, some important traits that have been claimed to be mostly genetically determined \revpoint{1}{14}
...
As also discussed in \citet{turelli2017commentary}, \revpoint{1}{8}
...
We refer to the first term on the left hand side as the \emph{reproduction term} and the remaining sum as the \emph{noise term}, and only consider the case \revpoint{2}{8}
...
\[
	...
	\end{cases}
\]
If $\gamma > 0$, then we recognize $e^{\psi(\theta)} = e^{im \theta -\gamma|\theta|}$
as the characteristic function of a Cauchy distribution with median 0 and median absolute deviation $\gamma$
(\ie, the canonical $\alpha$-stable distribution with $\alpha=1$), \llabel{cauchy_note}
whereas if $\gamma = 0$, then $e^{\psi(\theta)} = e^{im \theta}$ is the Fourier transform of a Dirac mass at $m$, \ie the characteristic function for a monomorphic population with all individuals having trait value equal to $m$.

% This might seem a bit roundabout, but the point is that we can prove that $e^{\psi(\theta)}$ is -- in this case -- a characteristic function, so we don't need to lose generality by assuming that it is one.  Our goal is to ask when $\lim_{n \to \infty} \varphi\left(\frac{\theta}{2^{n}}\right)^{2^{n}}\prod_{k=0}^{n-1}\Phi\left(\frac{\theta}{2^{k}}\right)^{2^{k}}$ is a valid characteristic function, and to see what it can be.  
To exclude the case $\gamma < 0$, we first observe that if, as we assume, $\varphi(\theta)$ is the characteristic function of some random variable, say $X$, then $\varphi_{n}(\theta) = \varphi\left(\frac{\theta}{2^{n}}\right)^{2^{n}} = e^{2^{n}\psi\left(\frac{\theta}{2^{n}}\right)}$ is the characteristic function of $2^{-n} \sum_{i = 1}^{2^{n}} X_{i}$, where $X_{1},\ldots,X_{2^{n}}$ are i.i.d.\ copies of $X$; now, L\'evy's continuity theorem (see \eg Theorem 2.3.4 in \cite{Durrett2005}) tells us that if (i) $\phi_{n}(\theta)$ has a point-wise limit, say $\phi_{\infty}(\theta)$, and (ii) $\phi_{\infty}(\theta)$ is continuous at 0, then $\phi_{\infty}(\theta)$ is the characteristic function of \emph{some} random variable; in particular, if $\alpha(\theta)$ exists and is bounded (\eg if the left and right-hand side derivatives exist, as above), then $\psi(\theta) = \alpha(\theta)\theta$ is continuous at 0, and hence so is $e^{\psi(\theta)}$.  Thus, in this case $e^{\psi(\theta)}$ is itself a characteristic function.  Moreover, being a characteristic function, $e^{\psi(\theta)}$ must satisfy $|e^{\psi(\theta)}| = e^{\Re \psi(\theta)} \leq 1$, whence $\gamma \geq 0$.
...
To see what knowing the trait value $Z^{[j]}_{M}$ \revpoint{1}{16}
...
converges weakly as $M \to \infty$ to a Poisson random variable with mean $2/(\pi \epsilon)$ (see \eg Theorem 2.6.1 in \cite{Durrett2005}).  In particular, for large values of $M$ and fixed $\epsilon > 0$,\revpoint{1}{17}
\begin{align*}
...
By contrast, in \citet{Fisher1918} and \citet{barton2017infinitesimal} \revpoint{2}{10}, the scaled allelic effect at any locus $\ell$ has magnitude bounded by $\frac{B}{\sqrt{M}}$, and thus, no matter how many alleles one considers, there will be no large effect alleles that are informative of the trait value.  


To summarize, the key argument in \citet{Fisher1918} and \cite{barton2017infinitesimal}  \revpoint{1}{10}
...
(e.g., \citet{lande1983response} and \citet{chevin2008selective}). \revpoint{1}{20}
