...
\[
	...
	\end{cases}
\]
If $\gamma > 0$, then we recognize $e^{\psi(\theta)} = e^{im \theta -\gamma|\theta|}$ as the characteristic function of a Cauchy distribution with median 0 and median absolute deviation $\gamma$, whereas if $\gamma = 0$, then $e^{\psi(\theta)} = e^{im \theta}$ is the Fourier transform of a Dirac mass at $m$, \ie the characteristic function for a monomorphic population with all individuals having trait value equal to $m$.
...
Also, we show that removing big effects and using the infinitesimal model for the remainder is good \comment{todo incorporate this}.
