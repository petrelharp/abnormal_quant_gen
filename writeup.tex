...
for a Cauchy random variable with median (location) $x_{0}$ and mean absolute deviation (scale) $\gamma$, the density is $f(x;x_{0},\gamma) = \frac{1}{\pi\gamma\left[1+\left(\frac{x-x_{0}}{\gamma}\right)^{2}\right]}$.  Armed with this density function, we can explicitly compute the right hand side of \eqref{eqn:MSB_CF}, assuming that 
\begin{enumerate}[(i)]
