...
using the Skitovich-Darmois theorem \citep{kagan1973characterization},
except that the distribution of the portion of the genome shared in common by the parents
is not constrained.
However, there is little to be gained by the additional generality.

Finally, note that we follow \citet{barton2017infinitesimal}
by assuming that loci in the founding generation are independent
...
Of course, following the offspring foward through a pedigree
(as \citet{barton2017infinitesimal} do),
the loci will no longer be independent.
One might then wonder if by relaxing the assumption of independence between loci
...
in particular,
we are \emph{not} working with the explicit sum-of-$\eta_\ell$ model of the previous sections.)
...
Mutation-selection balance demands that
