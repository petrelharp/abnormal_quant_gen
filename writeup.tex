...
\begin{abstract}
    ...
    suggesting that non-Gaussian models of trait evolution must expicitly
...
$$
The \emph{transmission noise}, $R_M$, is the offspring's trait minus the midparent value, $\bar Z_M$:
\begin{align*}
...
\begin{lem}\label{lem:parental_contribs}
    ...
    (i.e., $R_M$ is independent of $\bar Z_M$)
...
except that, unsurprisingly, the median trait is no longer constrained to wander
...
and ``segregation noise'' computed using only mutations with effect size less than 0.2
...
Not assuming all effects are the same order of magnitude seems very bioloically realistic,
and so it'd be nice to figure out how evolution works and how to do quant gen in such cases.
Such models can always be simulated,
but the way forwards with theoretical understanding seems murky.
It would be interesting to see how changing the distribution of segregation noise
in the infinitesimal model
affects properties like the rate of adaptive evolution,
levels of genetic load,
and the rate of fixation.
However,
we have seen that this assumption imples a sort of ``blending inheritance''
for the large effect alleles,
and so such work much be accompanied by simulations
...
Above we've mostly used the Cauchy (e.g., in simulations)
not because it seems most realistic
(indeed data suggests $\alpha=3/2$ might be better)
