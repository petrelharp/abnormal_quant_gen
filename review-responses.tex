...
\reply{
    ...
    (in \citet{fisher1918correlation} and \cite{barton2017infinitesimal})
...
\reply{
There isn't an analogous result in Barton et al., as they have assumed that all allele effects are bounded random variables rescaled by a factor of $\frac{1}{\sqrt{M}}$.  Thus, no matter how many alleles that one considers, none will have the large effects necessary to give information about that trait value.  We've added some text to clarify this difference at \revref.
}
...
\reply{
    That's right; we've edited this bit, which no longer says ``Lévy''. \revref
}
